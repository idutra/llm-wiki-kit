---
name: wiki
description: Maintain an LLM Wiki (Karpathy pattern) - a git-versioned markdown knowledge base compiled by the agent from immutable sources in raw/. One entry point for every write operation - init or adopt a wiki, ingest a source, archive an answer, lint and health-check, rebuild index.md / manifest.md / log.md, set up search, export decks and reports, publish the wiki as a read-only skill for other repositories, and change the schema. Use when the user says "init wiki", "criar wiki", "ingest", "ingerir", "adicionar ao wiki", "arquivar esta resposta", "lint wiki", "saude do wiki", "regenerar indice", "gerar o manifesto", "a fonte mudou", "configurar busca", "exportar", "slides do wiki", "publicar o wiki", "publish wiki", "convencoes do wiki", "alterar o schema", or drops a file into raw/. For read-only questions about the wiki content, prefer the lighter wiki-query skill.
license: MIT
metadata:
  author: idutra
  version: "0.2.0"
...
4. **Só o agente principal escreve, e escreve uma vez.** Subagentes (`wiki-ingest-agent`, `wiki-query-agent`, `wiki-lint-agent`, `wiki-curator`) são somente leitura e devolvem propostas.
